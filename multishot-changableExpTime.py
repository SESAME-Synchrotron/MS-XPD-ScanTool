...
class XRD:
	def __init__(self):
		...
		self.parser.add_argument('-exp',		type=float, default=5.0,    help='Initial exposure time (seconds)')
		self.parser.add_argument('-expInc',		type=float, default=5.0,    help='Exposure time increment size')
	...
	def scan(self):
		...
		self.expname = "img_{}_exp{}_N{}.tiff".format(datetime.now().strftime("%H%M%S.%f"), str(self.expTime), str(0))
		...
		for img in range(self.numImages):
			...
			self.expname = "img_{}_exp{}_N{}.tiff".format(datetime.now().strftime("%H%M%S.%f"), str(self.expTime), str(img+1))
			self.pvs["ImgName"].put(self.expname)
